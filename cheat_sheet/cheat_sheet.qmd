...
![](images/Screenshot%202023-05-24%20at%205.44.10%20PM.png)
...
``` r
haven::read_sas("filepath/filename.sas7bdat", "filepath/catalog.sas7bcat")
```
...
1)  One can use the `cut()` function to create a new categorical variable with cutpoints at the `breaks =` values. For example, say you have a variable with a minimum of 0 and a maximum of 100 with some missing values and you would like to make three categories defined by the values (0,x\] (category 1), (x,y\] (category 2), and (y,100\] (category 3). For the `breaks =`argument, you will want to supply the left- hand breaks: a value lower than the minimum, x, y and then a value larger than the maximum. (You can use `-Inf` and `Inf` if you don't know what the minimum and maximum are.) You can also use the `include.lowest =` and the `right =` arguments to change how the intervals are created.
    ...
    ``` r
    dataset$newvar <- cut(dataset$newvar, breaks = c(-Inf, x, y, Inf))
    ```
...
``` r
nquantiles <- 3
dataset$newvar <- cut(dataset$oldvar, breaks = quantile(dataset$oldvar, probs = seq(0, 1, length.out = nquantiles + 1)), include.lowest=TRUE)
```
...
``` r
newvars <- as.data.frame(model.matrix(~varname + othervar, data = dataset))
```
...
``` r
dataset$newvar <- factor(dataset$oldvar, levels = c(0, 1, 2), labels = c("small", "medium", "large"))
```
...
``` r
dataset$newvar <- forcats::fct_relevel(dataset$oldvar, "small", "medium", "large"))
```

To rename the levels "sm", "md", and "lg":

``` r
dataset <- forcats::fct_recode(dataset$oldvar, "sm" = "small", "md" = "medium", "lg" = "large"))
```

If the variable is numeric, you need to turn it into a character or factor variable before using `fct_recode()`:

``` r
dataset <- forcats::fct_recode(as.character(dataset$oldvar), "sm" = "0", "md" = "1", "lg" = "2"))
```
...
``` r
epitools::epitab(dataset$exposure, dataset$outcome, method = "riskratio")
```
...
``` r
dplyr::summarize(dataset, mean_1 = mean(varname1), sd_2 = sd(varname2), .by = categoricalvariable)
```
...
``` r
t.test(continuousvar ~ categoricalvar, data = dataset, var.equal = FALSE)
```
...
``` r
cor(dataset[, c("varname1", "varname2", "varname3")], method = "spearman")
```
...
``` r
model <- glm(outcome ~ exposure + var1 + var2, data = dataset, family = binomial())
```
...
``` r
model <- glm(outcome ~ exposure + factor(var1) + var2, data = dataset, family = binomial())
```
...
model <- glm(outcome ~ exposure + newvar, data = dataset, family = binomial())
```
...
``` r
library(dplyr)
models <- dataset |> group_by(categoricalvar) |> group_map(~glm(outcome ~ exposure, data = .x))
```
...
``` r
km <- survfit(Surv(timetooutcome, outcome) ~ 1, data = dataset)
plot(km)
```
...
``` r
km <- survfit(Surv(timetooutcome, outcome) ~ exposure, data = dataset)
plot(km)
```
...
``` r
survdiff(Surv(timetooutcome, outcome) ~ exposure, data = dataset)
```
...
``` r
survmire::ggsurvplot(km, data = dataset, pval = TRUE, pval.method = TRUE,
...
``` r
cox <- coxph(Surv(timetooutcome, outcome) ~ exposure, data = dataset)
```
...
``` r
fg_data <- finegray(Surv(timetooutcome, outcome) ~ exposure, data = dataset)
fg_model <- coxph(Surv(fgstart, fgstop, fgstatus) ~ exposure, weight = fgwt, data = fg_data)
```
